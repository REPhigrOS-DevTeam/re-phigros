...
namespace Schwarzer.Windows
{
    namespace Internal
    {
        ...
        public class OpenFileName
        {
            ...
            [MarshalAs(UnmanagedType.LPTStr)] public string filter = null;
            [MarshalAs(UnmanagedType.LPTStr)] public string customFilter = null;
            public int maxCustFilter = 0;
            public int filterIndex = 0;
            [MarshalAs(UnmanagedType.LPTStr)] public string file = null;
            public int maxFile = 0;
            [MarshalAs(UnmanagedType.LPTStr)] public string fileTitle = null;
            public int maxFileTitle = 0;
            [MarshalAs(UnmanagedType.LPTStr)] public string initialDir = null;
            [MarshalAs(UnmanagedType.LPTStr)] public string title = null;
            public int flags = 0;
            public short fileOffset = 0;
            public short fileExtension = 0;
            [MarshalAs(UnmanagedType.LPTStr)] public string defExt = null;
    ...
    public static class Dialog
    {
        ...
        private static string ParseEditorFilter(string Filter)
        {
            if (Filter == null) return "";
            return string.Join(",",
                Filter.Split('|')
                    .Where((_, i) => i % 2 == 1)//取出第偶数项，也就是表示后缀的那项
                    .Select(str => str.Substring(str.LastIndexOf('.') + 1)));
        }

        public static string OpenFileDialog(string Title = null, string Filename = null, string InitPath = null,
            string Filter = null)
        {
#if UNITY_EDITOR
            return EditorUtility.OpenFilePanel(Title, InitPath, ParseEditorFilter(Filter));
#endif
        ...
        public static string SaveFileDialog(string Title = null, string Filename = null, string InitPath = null,
            string Filter = null)
        {
#if UNITY_EDITOR
            return EditorUtility.SaveFilePanel(Title, InitPath, Filename, ParseEditorFilter(Filter));
#endif
            string filename = (Filename ?? "") + new string(new char[1024]);
            OpenFileName ofn = new OpenFileName
            {
