...
namespace Lean.Transition
{
    ...
    public class LeanTransition : MonoBehaviour
    {
        ...
        [SerializeField] [UnityEngine.Serialization.FormerlySerializedAs("Timing")]
        private LeanTiming defaultTiming = LeanTiming.UnscaledUpdate;
        ...
        public static int Count
        {
            get
            {
                return unscaledUpdateStates.Count + unscaledLateUpdateStates.Count + unscaledFixedUpdateStates.Count +
                       updateStates.Count + lateUpdateStates.Count + fixedUpdateStates.Count;
            }
        }
        ...
        public static void RequireSubmitted()
        {
            ...
            {
                Debug.LogError("You forgot to submit the last transition! " + currentQueue.GetType() + " - " +
                            ...
                            if (aliasTypePairs.TryGetValue(alias, out existingType) == true)
                            {
                                ...
                                if (existingType != targetType)
                                {
                                    ...
                                    Debug.LogError(
                                        ...
                                        alias + "), but these transitions use different types (" + existingType +
                                        ") + (" + targetType + "). You must give them different aliases.", root);
                                }
                            }
                            // Add new?
                            else
        ...
        private void RemoveConflictsBefore(List<LeanState> states, LeanState currentState, int currentIndex)
        {
            ...
            if (currentConflict != LeanState.ConflictType.None)
            {
                ...
                for (var i = 0; i < currentIndex; i++)
                {
                    ...
                    if (transition.Ignore == false && transition.GetType() == currentType &&
