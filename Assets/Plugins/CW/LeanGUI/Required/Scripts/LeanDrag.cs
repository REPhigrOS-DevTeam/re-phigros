...
namespace Lean.Gui
{
    ...
    public class LeanDrag : LeanSelectable, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        [System.NonSerialized] protected bool dragging;

        [System.NonSerialized] private Vector2 startOffset;
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (MayDrag(eventData) == true)
            {
                ...
                if (RectTransformUtility.ScreenPointToLocalPointInRectangle(target, eventData.position,
                    eventData.pressEventCamera, out vector) == true)
                {
                    dragging = true;
            ...
            if (dragging == true)
            {
                ...
                if (MayDrag(eventData) == true)
                {
                    ...
                    if (RectTransformUtility.ScreenPointToLocalPointInRectangle(target,
                        eventData.position - eventData.delta, eventData.pressEventCamera, out oldVector) == true)
                    {
                        var newVector = default(Vector2);

                        // Get the current pointer position relative to this rect transform
                        if (RectTransformUtility.ScreenPointToLocalPointInRectangle(target, eventData.position,
                            eventData.pressEventCamera, out newVector) == true)
                        {
