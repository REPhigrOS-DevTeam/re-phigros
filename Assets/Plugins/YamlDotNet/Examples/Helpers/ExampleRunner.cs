            ...
            foreach (Type t in Assembly.GetExecutingAssembly().GetTypes())
            {
                if (t.Namespace == "YamlDotNet.Samples" && t.IsClass && Array.IndexOf(disabledTests, t.Name) == -1)
                {
                    ...
                    foreach (MethodInfo mi in t.GetMethods())
                    {
                        ...
                        {
                            SampleAttribute sa =
                                (SampleAttribute) Attribute.GetCustomAttribute(mi, typeof(SampleAttribute));
                            if (sa != null)
                            {
                                helper.WriteLine("{0} - {1}", sa.DisplayName, sa.Description);
                                var testObject = t.GetConstructor(new Type[] {typeof(StringTestOutputHelper)})
                                    .Invoke(new object[] {helper});
